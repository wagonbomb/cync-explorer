#!/usr/bin/env python3
"""
Linux BLE Test v3 - Proper service discovery
"""
...
async def main():
    ...
    async with BleakClient(device, timeout=30.0) as client:
        ...
        for service in services:
            if "1910" in service.uuid or "1827" in service.uuid or "1828" in service.uuid:
        ...
        if any(responses):
            ...
            print()
            print("Trying control commands anyway...")
            for name, cmd in [("ON", "b0c00101"), ("OFF", "b0c00100")]:
                print(f"\n[{name}] -> {cmd}")
                try:
                    await client.write_gatt_char(UUID_1912, bytes.fromhex(cmd), response=False)
